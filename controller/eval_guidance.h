...
inline evaluateGuidance evaluateGuidanceMetrics(
    const std::vector<guidanceLog>& actual, const std::vector<ReferenceTraj>& ref_traj)
    {
        ...
        for (int i = 0; i < actual.size() - 1; ++i) {
            ...
            double dx = actual[i].mpc_log.x - nearest_traj.x;
            double dy = actual[i].mpc_log.y - nearest_traj.y;
            ...
            double dtheta = actual[i].mpc_log.theta - nearest_traj.theta;
            while (dtheta > M_PI) dtheta -= 2 * M_PI;
            while (dtheta < -M_PI) dtheta += 2 * M_PI;
            sum_sq_he += dtheta * dtheta;
            if (std::abs(dtheta) > 170*M_PI/180)
                std::cout << "" << std::endl;
            ...
            double effort = (actual[i].mpc_log.a * actual[i].mpc_log.a) + 
                            (actual[i].mpc_log.delta_dot * actual[i].mpc_log.delta_dot);
